...
// --- UI Update Functions ---
function displayPlayerHand(playerId, showFace = true) { const handDiv = document.getElementById(`hand-${playerId}`); handDiv.innerHTML = ''; const hand = playerHands[playerId]; hand.forEach(tile => { const tileElement = showFace ? createTileImageElement(tile) : createTileBackImageElement(); if (playerId === 'south' && mustPlayerDiscard && showFace && !playerTingStatus.south && !isGameOver ) { tileElement.classList.add('clickable'); tileElement.addEventListener('click', () => { if (mustPlayerDiscard && PLAYER_IDS[currentPlayerIndex] === 'south' && !isGameOver) { processPlayerDiscard('south', tile.uniqueId); } }, { once: true }); } handDiv.appendChild(tileElement); }); displayMelds(playerId); displayFlowers(playerId); }
